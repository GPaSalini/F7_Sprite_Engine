...
static void DMA2D_DrawImage(Sprite_Image *img, uint32_t xi, uint32_t yi, uint32_t xf, uint32_t yf, uint32_t width, uint32_t height);
static void DMA2D_FillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t col);
...
static void DMA2D_CopyBuffer(uint32_t *pSrc, uint32_t *pDst)
{
	...
	HAL_DMA2D_Start_IT(&hdma2d,(uint32_t)pSrc,(uint32_t)pDst,BSP_LCD_GetXSize(),BSP_LCD_GetYSize());
...
static void DMA2D_DrawImage(Sprite_Image *img, uint32_t xi, uint32_t yi, uint32_t xf, uint32_t yf, uint32_t width, uint32_t height)
{
	uint32_t source = (uint32_t)img->data + (xi + yi*img->width)*4;
	uint32_t destination = RENDER_BUFFER + (xf + yf*BSP_LCD_GetXSize())*4;
	...
	hdma2d.LayerCfg[1].InputOffset 		= img->width - width;
	...
	HAL_DMA2D_BlendingStart(&hdma2d,source,destination,destination,width,height);
...
static void DMA2D_FillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t col)
{
	uint32_t destination = RENDER_BUFFER + (x + y*BSP_LCD_GetXSize())*4;

	HAL_DMA2D_DeInit(&hdma2d);

	hdma2d.Init.Mode         = DMA2D_R2M;
	hdma2d.Init.ColorMode    = DMA2D_OUTPUT_ARGB8888;
	hdma2d.Init.OutputOffset = BSP_LCD_GetXSize() - w;
	hdma2d.Init.AlphaInverted = DMA2D_REGULAR_ALPHA;
	hdma2d.Init.RedBlueSwap   = DMA2D_RB_REGULAR;
	...
	HAL_DMA2D_Init(&hdma2d);
	HAL_DMA2D_Start_IT(&hdma2d,col,destination,w,h);
	HAL_DMA2D_PollForTransfer(&hdma2d,5);
}
...
void Frame_Sprites(void)
{
	...
	DMA2D_FillRect(0,0,BSP_LCD_GetXSize(),BSP_LCD_GetYSize(),C_Background);
...
void Step_Control(void)
{
	...
	if (BSP_LCD_GetXSize() > 720)
	{
		...
		#if(DEBUG_FPS==1)
		//Delta ticks
		itoa(ticklast-tickfirst,debug,10);
...
static void Draw_Sprite(Sprite_Type_Instance *ins, int16_t xi, int16_t yi)
{
	...
	for (uint8_t iscreen=0; iscreen<MAX_SCREEN; iscreen++)
	{
		if (Screen[iscreen].Activated)
		{
			...
			BOOL coll = FALSE;
			if (!(b > Screen[iscreen].height || c < 0 || d < 0 || a > Screen[iscreen].width)) coll = TRUE;
			...
			if (coll)
			{
				...
				uint16_t u = Screen[iscreen].width + Screen[iscreen].xScreen;
				uint16_t v = Screen[iscreen].height + Screen[iscreen].yScreen;
				...
				if (c>0 && d>0) DMA2D_DrawImage(obj->image,ofx,ofy,a,b,c,d);
			}
		}
	}
}

void Draw_FillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t col)
{
	DMA2D_FillRect(x,y,w,h,col);
}
...
void Draw_StringAt(uint16_t Xpos, uint16_t Ypos, uint8_t *Text)
{
	...
	if ((Xpos < 0) || (Xpos >= BSP_LCD_GetXSize()))
		return;
	...
	while ((*Text != 0) & (((BSP_LCD_GetXSize() - (i*Sprite_Font->Width)) & 0xFFFF) >= Sprite_Font->Width))
	{
		...
		Draw_Char(Xpos,Ypos,Ascii);
		Xpos += Sprite_Font->Width;
...
BOOL Collision_RectangleInstance(Sprite_Type_Instance* ins, uint16_t xa, uint16_t ya, uint16_t xb, uint16_t yb, int16_t xi, int16_t yi)
{
	...
	//Valores de posicion para la ins
	uint16_t insL = ins->x + xi;
	uint16_t insT = ins->y + yi;
	uint16_t insR = insL + Object_Get(ins->parent)->width -1;
	uint16_t insB = insT + Object_Get(ins->parent)->height -1;
	...
	if (recL>=recR || recT>=recB) return FALSE;

	//Revisa colisiones
	if (recT > insB || recR < insL || recB < insT || recL > insR)
	{
		return FALSE;
	}
	else return TRUE;
...
BOOL Collision_HitboxInstance(Sprite_Type_Instance* ins1, Sprite_Type_Instance* ins2, int16_t xi, int16_t yi)
{
	...
	uint16_t ins1L = ins1->x + Object_Get(ins1->parent)->hitbox.x + xi;
	uint16_t ins1T = ins1->y + Object_Get(ins1->parent)->hitbox.y + yi;
	uint16_t ins1R = ins1L + Object_Get(ins1->parent)->hitbox.w -1;
	uint16_t ins1B = ins1T + Object_Get(ins1->parent)->hitbox.h -1;

	//Valores de posicion para spr2
	uint16_t ins2L = ins2->x + Object_Get(ins2->parent)->hitbox.x;
	uint16_t ins2T = ins2->y + Object_Get(ins2->parent)->hitbox.y;
	uint16_t ins2R = ins2L + Object_Get(ins2->parent)->hitbox.w -1;
	uint16_t ins2B = ins2T + Object_Get(ins2->parent)->hitbox.h -1;

	//Revisa colisiones
	if (ins1T > ins2B || ins1R < ins2L || ins1B < ins2T || ins1L > ins2R)
	{
		return FALSE;
	}
	else return TRUE;
